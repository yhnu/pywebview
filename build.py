...
def get_release_file_name(ctx):
    ...
    if build_type == "release" and flavor == "public":
        return "PywebTableConvertTool-%s.zip" % (version)  # eg: PywebTableConvertTool-v1.0.0.zip
    else:
        return "PywebTableConvertTool-%s-%s-%s.zip" % (version, flavor, build_type)  # eg: PywebTableConvertTool-v1.0.0-internal-debug.zip
...
def build_package(ctx):
    ...
    shutil.copyfile("Perfeye.exe", os.path.join(nsis_dir, "PywebTableConvertTool.exe"))
